...
const BoardVisualization: React.FC<BoardVisualizationProps> = ({
  ...
  const leftHolePosition = shouldRotate ? {
    top: holeInsetX,
    left: holeInsetY
  } // When rotated, use left and top
  : {
    top: holeInsetY,
    left: holeInsetX
  }; // Normal position using top and left
  ...
  : {
    top: holeInsetY,
    right: holeInsetX
  }; // Normal position using top and right

  return <div className="board-visualization-container flex justify-center items-center my-8" ref={containerRef} style={{
    display: 'flex',
    alignItems: 'center',
    ...
    height: '100%'
      ...
      height: `${scaledWidth + scaledThickness + 60}px`
        ...
        boxShadow: shouldRotate ? `${scaledThickness}px 0 0 #a0826c` // Horizontal shadow when rotated
        : `0 ${scaledThickness}px 0 #a0826c`,
        // Vertical shadow when not rotated
