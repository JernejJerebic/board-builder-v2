...
const BoardVisualization: React.FC<BoardVisualizationProps> = ({
  ...
  const updateBoardVisualization = () => {
    ...
    if (shouldRotate) {
      boardRef.current.style.boxShadow = `${scaledThickness}px 0 0 ${color?.darkShadow || '#8a6a58'}`;
    } else {
      boardRef.current.style.boxShadow = `0 ${scaledThickness}px 0 ${color?.darkShadow || '#8a6a58'}`;
    }
  ...
  return (
    <div className="board-visualization-container flex justify-center items-center my-8" ref={containerRef} style={{
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      height: '100%'
    }}>
      <div className="board-wrapper relative" style={{
        height: `${scaledWidth + scaledThickness + 60}px`
      }}>
        <div ref={boardRef} className={`board-main relative ${shouldRotate ? 'board-rotated' : 'board-normal'}`} style={{
          ...
          boxShadow: shouldRotate ? `${scaledThickness}px 0 0 ${color?.darkShadow || '#8a6a58'}` : `0 ${scaledThickness}px 0 ${color?.darkShadow || '#8a6a58'}`,
          ...
          {adjustedBorders.top && <div className="board-border board-border-top absolute top-0 left-0 right-0 h-2 bg-gray-300 border border-gray-400 z-10" style={{
            transform: 'translateY(-1px)'
          }}></div>}
          {adjustedBorders.right && <div className="board-border board-border-right absolute top-0 bottom-0 right-0 w-2 bg-gray-300 border border-gray-400 z-10" style={{
            transform: 'translateX(1px)'
          }}></div>}
          {adjustedBorders.bottom && <div className="board-border board-border-bottom absolute bottom-0 left-0 right-0 h-2 bg-gray-300 border border-gray-400 z-10" style={{
            transform: 'translateY(1px)'
          }}></div>}
          {adjustedBorders.left && <div className="board-border board-border-left absolute top-0 bottom-0 left-0 w-2 bg-gray-300 border border-gray-400 z-10" style={{
            transform: 'translateX(-1px)'
          }}></div>}
        ...
        <div className="board-side board-side-right absolute bg-neutral-700 dark:bg-neutral-900" style={{
        ...
        <div className="board-side board-side-bottom absolute bg-neutral-800 dark:bg-neutral-950" style={{
          width: `${scaledLength}px`,
